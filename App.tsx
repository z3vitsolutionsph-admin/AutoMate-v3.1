...
const App: React.FC = () => {
  ...
  const saveToStorage = (key: string, value: any) => {
    try {
      ...
      console.warn(`[Storage Warning] Failed to persist data for ${key}:`, e);
      // In a real production app, we might want to trigger a toast here
    }
  };
  ...
  useEffect(() => {
    ...
    saveToStorage(KEYS.SETUP, 'true');
    saveToStorage(KEYS.BIZ_NAME, businessName);
    saveToStorage(KEYS.CATS, categories);
    ...
    saveToStorage(KEYS.INTEGRATIONS, integrations);
    saveToStorage(KEYS.PLAN, subscriptionPlan);
    
    if (trialStartDate) {
      saveToStorage(KEYS.TRIAL_START, trialStartDate);
    } else {
  ...
  const handleOnboardingComplete = useCallback((data: OnboardingState) => {
    ...
    if (data.selectedPlan === 'STARTER') {
       ...
       saveToStorage(KEYS.TRIAL_START, now);
    ...
    saveToStorage(KEYS.SESSION_USER, adminUser.id);
  }, []);
  ...
  const handleLogin = useCallback((email: string, pass: string) => {
    ...
    if (user) {
      ...
      saveToStorage(KEYS.SESSION_USER, user.id);
  ...
  const upgradeToProfessional = () => {
    ...
    saveToStorage(KEYS.PLAN, 'PROFESSIONAL');
    localStorage.removeItem(KEYS.TRIAL_START);
    setTrialStartDate(null);
  };
