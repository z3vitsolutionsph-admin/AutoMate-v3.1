...
import { Mail, ArrowRight, AlertCircle, ShieldCheck, Loader2, KeyRound, Box, Store, Bot, BarChart3, ArrowLeft, WifiOff, X, CheckCircle2 } from 'lucide-react';
...
export const Login: React.FC<LoginProps> = ({ onLoginSuccess, onBack, businessName }) => {
  ...
  const triggerShake = () => {
    setIsShaking(false); // Reset to allow re-triggering
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Artificial delay for UX consistency + Network request
      // We start the request immediately but ensure the spinner shows for at least 600ms to prevent layout flicker
      ...
      // If success, the parent component handles the redirect/state change, so we leave isLoading true
    } catch (err) {
      console.error(err);
            ...
            <div className="bg-white border border-slate-200/60 p-8 md:p-12 rounded-[2.5rem] shadow-[0_20px_60px_-15px_rgba(0,0,0,0.1)] relative overflow-hidden">
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="space-y-2 group">
                  ...
                  <div className="relative">
                    ...
                    />
                    {email && !isLoading && (
                      <button type="button" onClick={() => setEmail('')} className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-300 hover:text-slate-500 p-1">
                        <X size={16} />
                      </button>
                    )}
                  </div>
                ...
                <div className="space-y-2 group">
                  <div className="flex justify-between items-center ml-3 mr-1">
                     <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] group-focus-within:text-indigo-600 transition-colors">Secure Key</label>
                  </div>
                  <div className="relative">
                ...
                <div className="pt-2">
                  ...
                  >
                    {isLoading ? (
                      ...
                      <>
                        <span className="uppercase tracking-widest text-xs">Unlock Terminal</span> 
                        <ArrowRight size={18} />
                      </>
                    )}
                  </button>
              ...
              <div className="mt-10 pt-6 border-t border-slate-100 text-center flex flex-col gap-2">
                <p className="text-[9px] font-black text-slate-400 uppercase tracking-[0.3em] flex items-center justify-center gap-2">
                  <ShieldCheck size={10} /> Precision Core v3.4.0
                </p>
                <button onClick={() => alert("Please contact your organization administrator to reset your Secure Key.")} className="text-[10px] text-indigo-500 font-bold hover:text-indigo-700 hover:underline transition-all w-fit mx-auto">
                  Lost Access Key?
                </button>
              </div>
            </div>
